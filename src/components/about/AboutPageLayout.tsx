          ...
          <article>
            ...
            <div className="mt-5 space-y-4">
              {content.mission.items.map((item) => (
                <div key={item.title} className="bg-brand-100/40 rounded-[10px] p-5">
                  <h3 className="text-[20px] font-bold">{item.title}</h3>
                  <Body size="md" className="mt-2 text-[18px] leading-7 tracking-normal">
                    {item.description}
                  </Body>
                </div>
              ))}
        ...
        <section className="mt-12">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {content.milestones.events.map((event) => (
              <article key={event.year} className="border-brand-100 rounded-[10px] border p-5">
                <p className="text-brand-500 text-2xl font-bold">{event.year}</p>
                <Body size="md" className="mt-2 text-[18px] leading-7 tracking-normal">
                  {event.text}
                </Body>
              </article>
        ...
        <section className="mt-12">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {content.team.members.map((member) => (
              ...
              >
                <div className="mb-4 size-16 rounded-full bg-white/20" aria-hidden />
